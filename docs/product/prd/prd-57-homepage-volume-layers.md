...
This feature adds two new Experience-layer modules around Top 5 Signals: a Developing Now module and a By Category module. Both modules must consume the existing ranked briefing output and the existing `homepageClassification` / fallback taxonomy path without modifying ingestion, source governance, ranking, or classification rules. Top 5 Signals remains capped at five and keeps its current rendering path. Category controls render as demoted links at the top of the homepage with a `BROWSE BY` label; Technology, Finance, and Politics Article rows render only on their dedicated category pages. The first homepage render must not include category Article rows in the initial HTML.
...
By Category renders three subsections in the fixed order Technology, Finance, Politics. Each subsection selects up to three fresh events whose `classification.primaryCategory` matches the category key, while excluding anything already used in Top 5 Signals or Developing Now. Subsections do not borrow from one another. Politics preserves the existing honest empty-state rule rather than backfilling with technology or finance coverage.
...
When public source supply is healthy, the homepage prioritizes the Top 5 Signals in the initial render and leaves category news behind explicit reader intent. Developing Now must exclude Top 5 IDs, favor fresher items, prefer sources not already represented in Top 5, and return no semantic near-duplicates of already surfaced stories when that layer is rendered. By Category must render Technology, Finance, and Politics links in that order at the top of the homepage, restore the visible `BROWSE BY` label, and route to `/technology`, `/economics`, and `/politics` respectively. Each category page renders up to 12 non-paywalled Articles from the latest two cron runs for only that category. Article rows must include date, source, title, original-source link, and must not duplicate current top 5-7 Signal Card source URLs or titles. The existing Top 5 Signals path, public manifest, ranking logic, and Politics empty-state invariant must remain intact.
